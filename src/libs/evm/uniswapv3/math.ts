...
export function fromsqrtPriceX96ToHumanReadable(
  ...
  return {
    token0Token1Price: _price / (Number(expDecimals1) / Number(expDecimals0)), // symbol1/symbol0
    token1Token0Price:
...
export function calcCurrentReserves(
  ...
  if (tickCurrent < tickLower) {
    ...
    amount1 = Math.floor(Number(suppliedLiquidity) * (sqrtRatioB - sqrtRatioA));
  } else if (tickCurrent >= tickLower && tickCurrent < tickUpper) {
    amount0 = Math.floor(
      Number(suppliedLiquidity) *
        ((sqrtRatioB - sqrtPrice) / (sqrtPrice * sqrtRatioB))
    );
    amount1 = Math.floor(Number(suppliedLiquidity) * (sqrtPrice - sqrtRatioA));
  }
...
export function calcUncollectedFees(
  ...
  let uncollectedFees_0 =
    (suppliedLiquidity * _subIn256(fr_t1_0, feeGrowthInsideLastX128_0)) / Q128;
  let uncollectedFees_1 =
    (suppliedLiquidity * _subIn256(fr_t1_1, feeGrowthInsideLastX128_1)) / Q128;
